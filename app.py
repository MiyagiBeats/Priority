...
def analyze_campaign_performance(df):
    ...
    for index, row in df.iterrows():
        ctr = pd.to_numeric(row['CTR'].strip('%')) if isinstance(row['CTR'], str) else row['CTR']
        if ctr < 1.0:
...
def analyze_keyword_performance(df):
    recommendations = []
    for index, row in df.iterrows():
        cpc = pd.to_numeric(row['CPC']) if isinstance(row['CPC'], str) else row['CPC']
        if cpc > 1.0:
            recommendations.append({
                "keyword": row['Keyword'],
...
def analyze_audience_performance(df):
    recommendations = []
    for index, row in df.iterrows():
        cost_per_conversion = pd.to_numeric(row.get('Cost per Conversion', None))
        if cost_per_conversion and cost_per_conversion > 10:
            recommendations.append({
